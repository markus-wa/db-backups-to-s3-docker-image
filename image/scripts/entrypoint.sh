...
printenv | sed 's/^\([^=]*\)=\(.*\)$/export \1='\''\2'\''/g' | grep -E '^export [A-Z_]' > /etc/cron_env.sh
...
(echo "${BACKUP_SCHEDULE} /bin/bash -c '. /etc/cron_env.sh && /opt/backup.sh' >> /var/log/cron.log 2>&1"; echo "") | crontab -
